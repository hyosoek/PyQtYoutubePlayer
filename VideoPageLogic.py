...
class VideoPageLogic(QWidget):
    def __init__(self,Ui,playListCode):
        super().__init__()
        self.ui = Ui
        ...
        for i in range(0,len(videoData)): 
            self.addVideo(videoData[i][2]) #parameter로 url을 사용하지 않습니다.

        playListData = db.dataRead("playlist","playlistcode",self.playListCode)
        userData = db.dataRead("user","usercode",playListData[0][0])
        self.ui.videoPageidLabel.setText(userData[0][0]+"님 환영합니다!")
    ...
    def addVideo(self,videocode):
        ...
        videoBtn.setObjectName(str(videocode))
        
        db = DataBase()
        videoDB = db.dataRead("video","playlistcode",self.playListCode)
        for i in range(0,len(videoDB)):
            if videoDB[i][2] == videocode:
                videoUrl = videoDB[i][1]
    ...
    def removeVideo(self,event,url):
        db = DataBase()
        dataTemp = db.dataRead("video","playlistcode",self.playListCode)
        for i in range(0,len(dataTemp)):
            if dataTemp[i][1] == url:
                videoCode = dataTemp[i][2]
